...
async def clear_user_context(user_id):
	await asyncio.sleep(90)  # Ожидаем 1.5 минуты
	if user_id in user_contexts:
		del user_contexts[user_id]
		logging.info(f"Прошло 90 секунд. Контекст пользователя {user_id} очищен.")
...
async def handle_messages(message: types.Message):
	try:
		...
		# При необходимости, уведомляем пользователя о проблеме
		await message.answer(
			"Извините, произошла ошибка сервера. Мы работаем над решением проблемы. Отправьте ваше сообщение еще раз через пару минут.")
