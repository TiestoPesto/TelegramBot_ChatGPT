...
async def handle_messages(message: types.Message):
	try:
		...
		if chat_member.is_chat_member():
			...
			# Обновление контекста разговора пользователя
			...
			messagess = []
			messagess.append({'role': 'user', 'content': message.text})
			print(messagess)
			...
			messagess.append({'role': 'assistant', 'content': chat_completion.choices[0].message.content})
